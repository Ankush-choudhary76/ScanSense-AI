...
def save_memory_summary(session_id, summary):
    """Save or update a summary for a completed session."""
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute(
...
def get_recent_memory_summaries(exclude_session_id=None, limit=5):
    """Retrieve the most recent N session summaries for long-term context."""
    conn = sqlite3.connect(DB_NAME)
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
